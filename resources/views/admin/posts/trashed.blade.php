...
@section('content')
        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body table-responsive">
                    <h4 class="mt-0 header-title">Trashed Posts</h4>
                        ...
                        @foreach($posts as $post)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a data-bs-toggle="modal"  data-bs-target="#restoreModal_{{$post->id}}" data-action="{{ route('posts.restore', $post->id) }}" class="btn btn-outline-danger"><i class="fe-refresh-ccw"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
    ...
    @foreach($posts as $post)
        <!-- Restore Modal -->
        <div class="modal fade" id="restoreModal_{{$post->id}}" data-backdrop="static" tabindex="-1" role="dialog"
             aria-labelledby="deleteUserModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteUserModalLabel">Restore Post?</h5>
                    ...
                    <form action="{{ route('posts.restore', $post->id) }}" method="POST">
                        <div class="modal-body">
                            @csrf
                            @method('DELETE')
                            <p class="text-center">Are you sure you want to restore this post with title <strong>{{ $post->title }}</strong>?</p>
                        </div>
